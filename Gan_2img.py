...
# netD.load_state_dict(torch.load('gan_faces/netD_epoch_%d.pth'%resume_epoch))
netG = _netG()
netG.apply(weights_init)
# netG.load_state_dict(torch.load('gan_faces/netG_epoch_%d.pth'%resume_epoch))
...
for epoch in range(niter):
    for i, data in enumerate(dataloader, 0):
        ...
        if i % 100 == 0:
            ...
            errGl.append(round(errG.cpu().detach().numpy().tolist(), 4))
            errDl.append(round(errD.cpu().detach().numpy().tolist(), 4))
            # do checkpointing
...
epochl = [i+1 for i in range(len(errGl))]

plt.title('Result Analysis')
plt.plot(epochl, errGl, color='green', label='errGl')
plt.plot(epochl, errDl, color='red', label='errDl')
plt.show()
